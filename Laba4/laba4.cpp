...
void FindOptimalThreads(int matrix_size) {
    ...
    for (size_t m = 0; m < init_funcs.size(); m++) {
        ...
        cout << left << setw(12) << "Потоки" << " | " << setw(15) << "Время (мс)" << " | " << setw(15) << "Ускорение" << endl;
    ...
    for (const auto& res : cpu_results) {
        cout << left << setw(25) << res.name << " | " << setw(28) << res.optimal_threads << " | " << fixed << setprecision(2) << res.max_speedup << "x" << endl;
    }
    // Находим лучший CPU метод
    int best_cpu_idx = 0;
...
int main(int argc, char* argv[]) {
    ...
    vector<int> optimal_sizes = {500, 1000, 2000, 3000, 4000, 5000};
